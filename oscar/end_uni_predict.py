...
from oscar.modeling.modeling_bert_infer import BertForImageCaptioning
...
class CaptionTSVDataset(Dataset):
    def __init__(self, args, yaml_file, tokenizer=None, add_od_labels=True,
        ...
        # /data1/yll/DistImgCap/process_data/gen_prompts/vinvl_result_nounEnd_prompts.json
        self.caption_file = args.caption_file

        print('self.caption_file', self.caption_file)
        ...
        # each data term in self.captions
        # e.g. {'image_id': '318556', 'id': 48, 'caption': 'A very clean and well decorated empty bathroom'}
        ...
        self.tensorizer = CaptionTensorizer(self.tokenizer, max_img_seq_length,
                ...
                is_train=is_train, n_ctx=self.n_ctx)
        self.add_od_labels = add_od_labels
        self.is_train = is_train
        self.kwargs = kwargs
        self.image_keys = self.prepare_image_keys()
        self.key2index, self.feat_key2index = self.prepare_image_key_to_index()
    ...
    def prepare_image_key_to_index(self):
        label_key2idx = {self.label_tsv.seek(i)[0] : i for i in tqdm(range(self.label_tsv.num_rows()))}
        feat_key2idx = {self.feat_tsv.seek(i)[0] : i for i in tqdm(range(self.feat_tsv.num_rows()))}
        return label_key2idx, feat_key2idx
    ...
    def get_image_features(self, img_key):
        img_idx = self.feat_key2index[img_key]
        row = self.feat_tsv.seek(img_idx)
        if len(row) == 3:
            img_key, num_boxes, feat_info_features = row[0], row[1], row[2]
            features = np.frombuffer(base64.b64decode(feat_info_features), np.float32
                    ).reshape((int(num_boxes), -1))
        else:
    ...
    def __getitem__(self, idx):
        ...
        features = self.get_image_features(img_key)
...
class CaptionTensorizer(object):
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None,
        ...
        text_a_gcap = text_a.split('.')[0]
        text_a_detail = text_a.split('.')[1]

        text_a = text_a_gcap + '. ' + ' '.join(['X'] * self.n_ctx) 
        tokens_a = self.tokenizer.tokenize(text_a)
        prompt_sid = tokens_a.index('.') + 1 
        ...
        prompt_sid += 1
        prompt_sid = torch.tensor([prompt_sid], dtype=torch.long)

        # pad text_a to keep it in fixed length for better inference.
        padding_a_len = self.max_seq_a_len - seq_a_len
        tokens += [self.tokenizer.mask_token] * padding_a_len
        ...
        if self.is_train:
            ...
            return (input_ids, attention_mask, segment_ids, img_feat, masked_pos, masked_ids, prompt_sid)
        return (input_ids, attention_mask, segment_ids, img_feat, masked_pos, prompt_sid)
...
def build_dataset(yaml_file, tokenizer, args, is_train=True):
    ...
    # pdb.set_trace()
    if is_train:
        return CaptionTSVDataset(args, yaml_file, tokenizer=tokenizer,
    ...
    return dataset_class(args, yaml_file, tokenizer=tokenizer,
            add_od_labels=args.add_od_labels, max_img_seq_length=args.max_img_seq_length,
...
def make_data_loader(args, yaml_file, tokenizer, is_distributed=True, 
    ...
    # data_loader = torch.utils.data.DataLoader(
    #     dataset, num_workers=0, sampler=sampler,
    #     batch_size=images_per_gpu,
    #     pin_memory=True,
    # )
    return data_loader
...
def train(args, train_dataloader, val_dataset, model, tokenizer):
    ...
    grouped_parameters = [
        {'params': [p for n, p in model.named_parameters() if not \
            any(nd in n for nd in no_decay) and 'prompt_learner' in n], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in model.named_parameters() if \
            any(nd in n for nd in no_decay) and 'prompt_learner' in n], 'weight_decay': 0.0}
    ]
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, (img_keys, batch) in enumerate(train_dataloader):
            ...
            if not args.scst:
                ...
                inputs = {'input_ids': batch[0], 'attention_mask': batch[1],
                    ...
                    'masked_pos': batch[4], 'masked_ids': batch[5], 'prompt_sid': batch[6]
                }
        ...
        # save checkpoit and evaluate per epoch
...
def test(args, test_dataloader, model, tokenizer, predict_file):
    ...
    inputs_param = {'is_decode': True,
        'do_sample': False,
        'bos_token_id': cls_token_id, # 101
        'pad_token_id': pad_token_id, # 0
        'eos_token_ids': [sep_token_id], # 102
        ...
        "top_k": args.top_k,
        "top_p": args.top_p,
        "repetition_penalty": args.repetition_penalty, # 1
        "length_penalty": args.length_penalty, # 1
        "num_return_sequences": args.num_return_sequences, # 1
        "num_keep_best": args.num_keep_best, # 1
    }
    if args.use_cbs: # false
        inputs_param.update({'use_cbs': True,
            'min_constraints_to_satisfy': args.min_constraints_to_satisfy,
        })
    def gen_rows():
        time_meter = 0
        save_path = op.join(args.output_dir, args.output_file)
        if op.exists(save_path):
            os.remove(save_path)
        json_out = open(save_path, 'a+')
        
        save_ids = []
        with torch.no_grad():
            for step, (img_keys, batch) in tqdm(enumerate(test_dataloader)):
                if img_keys in save_ids:
                    continue
                save_ids.append(img_keys)
                batch = tuple(t.to(args.device) for t in batch)
                '''
                'input_ids': text_a 20 words + od_label 30 words   [CLS, MASK, MASK, ... , MASK, SEP] + [labels ... ] 
                'img_feats': shape [50, 2054] 
                'token_type_ids': [0,0,..,1,1,1,0,0,0]  valid od_labels pos = 1
                'masked_pos': [1,1,...,1]  each elem = 1
                '''
                inputs = {
                    'input_ids': batch[0], 'attention_mask': batch[1],
                    'token_type_ids': batch[2], 'img_feats': batch[3],
                    'masked_pos': batch[4], 'prompt_sid': batch[5]
                }
                if args.use_cbs: # false
                    inputs.update({
                        'fsm': batch[5],
                ...
                for img_key, caps, confs, input_text in zip(img_keys, all_caps, all_confs, inputs['input_ids']):
                    res = []
                    for cap, conf in zip(caps, confs):
                        cap = tokenizer.decode(cap.tolist(), skip_special_tokens=True)
                        res.append({'caption': process(cap), 'conf': conf.item(), 'prompt_gen': cap})
                        
                    if isinstance(img_key, torch.Tensor):
                        img_key = img_key.item()

                    text_a = input_text[:args.max_gen_length]
                    prompt = tokenizer.decode(text_a.tolist(), skip_special_tokens=True)
                    json_out.write(json.dumps({'image_id':img_key, 'caption': process(cap), 'raw_cap': cap, 'prompt': prompt, 'conf': conf.item()}))
                    json_out.write('\n')
...
def restore_training_settings(args):
    if args.do_train:
        ...
        assert args.do_test or args.do_eval or args.do_predict
...
def get_world_size():
    if not dist.is_available():
        print("not dist.is_available()")
        return 1
    if not dist.is_initialized():
        print("not dist.is_initialized()")
        return 1
...
def main():
    ...
    parser.add_argument("--caption_file", default='', type=str, required=False,
                        help="The prompt file.")
    parser.add_argument("--train_yaml", default='train.yaml', type=str, required=False, 
                        help="yaml file for training.")
    parser.add_argument("--predict_yaml", default='test.yaml', type=str, required=False,
    ...
    parser.add_argument("--output_file", default='prompt2cap.json', type=str, required=False,
                        help="The output file to save genearted results.")
    ...
    # for soft prompts learning
    parser.add_argument('--n_cls', type=int, default=4,
                        help="number of type {REL, ATTR, NOBJ, BKG}")
    parser.add_argument('--n_ctx', type=int, default=2,
                        help="prompt vectors num")      
    parser.add_argument('--ctx_init', type=str, default='',
                        help="prompt vectors initialization")              
    parser.add_argument('--use_CSC', action='store_true',
                        help='Use class specific context prompts')
    ...
    config.n_cls = args.n_cls
    config.n_ctx = args.n_ctx
    config.ctx_init = args.ctx_init
    config.use_CSC = args.use_CSC
    ...
    if args.do_predict:
        logger.info("Evaluate on dataset: " + args.predict_yaml)
        test_dataloader = make_data_loader(args, args.predict_yaml,
