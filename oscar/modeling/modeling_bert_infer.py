...
from .modeling_utils_infer import CaptionPreTrainedModel, ImgPreTrainedModel
...
"""
add class prompt_learner
"""

class PromptLearner(nn.Module):
    def __init__(self, config=None, token_embedding=None):
        super().__init__()
        n_cls = config.n_cls  # 4 len({REL, ATTR, NOBJ, BKG})
        n_ctx = config.n_ctx  # 2 context prompts num
        ctx_init = config.ctx_init # random initialize or use word embeddings
        ctx_init_tokens = config.ctx_init_tokens
        ctx_dim = 768  # context prompts dim
        use_CSC = config.use_CSC
        dtype = 'torch.float32'
        init_std = config.init_std # 0.02
        # cands = ['the', 'man', 'woman', 'it', 'is', 'there', 'are', 'wear']

        if ctx_init:
            # use given words to initialize context vectors
            with torch.no_grad():
                token_type_ids = torch.zeros((1, n_ctx), dtype=torch.long)
                input_ids = ctx_init_tokens[:n_ctx]
                input_ids = torch.tensor(input_ids, dtype=torch.long).unsqueeze(0)
                word_embeddings = token_embedding(input_ids, position_ids=None,
                    token_type_ids=token_type_ids)
            ctx_vectors = word_embeddings.squeeze()
            ...
            if use_CSC:
                ...
                print("Initializing a generic context, std = {}".format(init_std))
        ...
        # ctx_vectors.shape = [16, 512]
        self.ctx = nn.Parameter(ctx_vectors)  # to be optimized

        self.n_cls = n_cls
        self.n_ctx = n_ctx
        cands = ['the', 'man', 'woman', 'it', 'is', 'there', 'are', 'wear']
        init_words = cands[:n_ctx]
        self.init_words = init_words
        # 'prefix': img tokens| [P] [P] | general cap | details
        # 'mid': img tokens | general cap | [P] [P] | details.
        self.prompt_position = 'mid' 

    def forward(self, input_ids, embeddings, prompt_sid):
        # input_ids  [48, 70] 
        # embeddings [48, 70, 768]
        # pdb.set_trace()
        ctx = self.ctx
        prompts = []
        if self.prompt_position == 'mid':
            for i in range(input_ids.shape[0]):
                prompts_i = []
                input_ids_i = input_ids[i].unsqueeze(0)
                embeddings_i = embeddings[i].unsqueeze(0)
                for j in range(input_ids_i.shape[0]):
                    pstart = prompt_sid[j].tolist()[0]  
                    pend = pstart + self.n_ctx 
                    prompt = torch.cat(
                        [
                            embeddings_i[j, :pstart],
                            ctx,
                            embeddings_i[j, pend:],
                        ],
                        dim=0
                    )
                    prompts_i.append(prompt.unsqueeze(0))
                prompts_i = torch.cat(prompts_i, dim=0)
...
class BertImgModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # add learnable prompt vectors
        self.prompt_learner = PromptLearner(config)


        self.img_dim = config.img_feature_dim
        logger.info('BertImgModel Image Dimension: {}'.format(self.img_dim)) # self.img_dim = 2054
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
            ...
            encoder_history_states=None, prompt_sid=None):
        ...
        # input_ids.shape [48, 70]
        # position_ids.shape None
        # token_type_ids.shape [48, 70]
        # embedding_output.shape [48, 70, 768]
        # pdb.set_trace()
        token_embeddings = self.embeddings(input_ids, position_ids=position_ids,
                token_type_ids=token_type_ids)
        embedding_output = self.prompt_learner(input_ids, token_embeddings, prompt_sid)
        ...
        if img_feats is not None:
            if self.img_feature_type == 'dis_code':
                ...
                # img_feats [48, 50, 2054]
                # img_embedding_output [48, 50, 768]
            ...
            # concatenate two embeddings (text&tag embeddings + img embeddings)   [48, 70, 768] + [48, 50, 768]
            # embedding_output [48, 120, 768]
            embedding_output = torch.cat((embedding_output, img_embedding_output), 1) # torch.Size([1, 43, 768])

        # extended_attention_mask [48, 1, 120, 120]
        encoder_outputs = self.encoder(embedding_output,
                extended_attention_mask, head_mask=head_mask,
                encoder_history_states=encoder_history_states)
        sequence_output = encoder_outputs[0] # [48, 120, 768]
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None, prompt_sid=None,
        ...
        outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask, 
                ...
                head_mask=head_mask,
                encoder_history_states=encoder_history_states, prompt_sid=prompt_sid)
        sequence_output = outputs[0][:, :masked_pos.shape[-1], :] # torch.Size([5, 46, 768])

        # pdb.set_trace()

        if is_training:
            sequence_output = outputs[0][:, :masked_pos.shape[-1], :] # [48, 70, 768]
    ...
    def prepare_inputs_for_generation(self, curr_ids, prompt_sid, past=None):
        ...
        return {'input_ids': input_ids, 'img_feats': img_feats,
            ...
            'is_training': False,
            'encoder_history_states': self.prev_encoded_layers, "prompt_sid": prompt_sid}
    ...
    inference algorithm
    """
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None, prompt_sid=None,
        ...
        self.img_seq_len = img_feats.shape[1] # 50
        self.max_seq_len = max_length # 20

        self.max_seq_a_len = 0
        ...
        if self.add_od_labels:
            ...
            for wi in range(input_ids.shape[1]):
                if input_ids[0][wi] == 103:
                    self.max_seq_a_len = wi
                    break
            input_ids = input_ids[:, :self.max_seq_a_len]
