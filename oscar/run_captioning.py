...
from oscar.utils.caption_evaluate import (evaluate_on_coco_caption, eval_distinct,
        ScstRewardCriterion)
...
class CaptionTSVDataset(Dataset):
    def __init__(self, args, yaml_file, tokenizer=None, add_od_labels=True,
        ...
        self.split = self.caption_file.split('/')[-1].split('_')[0]
        ...
        self.captions = []
        self.caption_file = args.caption_file.format(self.split) # './datasets/{}_prefix_prompts_flt.json'
        print(self.caption_file)
        if self.caption_file and op.isfile(self.caption_file):
            with open(self.caption_file, 'r') as f:
                self.captions = json.load(f)
        print("self.captions size:", len(self.captions))
    ...
    def __getitem__(self, idx):
        ...
        # print(img_key)
        # print(example[0].shape, example[1].shape, example[2].shape, example[3].shape, example[4].shape, example[5].shape, example[6].shape)
...
class CaptionTensorizer(object):
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None,
        ...
        if self.is_train:
            if len(text_a_detail) <= 1:
                text_a = ' '.join(['X'] * self.n_ctx) + ' ' + text_a
                tokens_a = self.tokenizer.tokenize(text_a)
                init_idx = self.n_ctx
            else:
                text_a = text_a_gcap + '. ' + ' '.join(['X'] * self.n_ctx) + text_a_detail + '.'
                tokens_a = self.tokenizer.tokenize(text_a)
                init_idx = tokens_a.index('.') + 1 + self.n_ctx
        else:
            # only input general cap tokens to generate end details
            text_a = text_a_gcap + '. ' + ' '.join(['X'] * self.n_ctx)
            tokens_a = self.tokenizer.tokenize(text_a)
            # fake tokens to generate masks
            tokens_a += [self.tokenizer.mask_token] * (self.max_seq_a_len - 2 - len(tokens_a))
            init_idx = tokens_a.index('.') + 1 + self.n_ctx
        ...
        init_idx += 1
        prompt_sid = init_idx - self.n_ctx
        ...
        if self.is_train:
            ...
            candidate_masked_idx = list(range(init_idx, seq_a_len)) # only mask text_a
            random.shuffle(candidate_masked_idx)
            num_masked = min(max(round(self.mask_prob * seq_a_len), 1), self.max_masked_tokens)
            num_masked = min(int(num_masked), len(candidate_masked_idx))
...
def train(args, train_dataloader, val_dataset, model, tokenizer):
    ...
    # pdb.set_trace()
    # print(dict(model.named_parameters())['bert.prompt_learner.ctx'])
    # print(dict(model.named_parameters())['bert.encoder.layer.1.attention.self.query.weight'])
    print('Turning off gradients in cross emcoder')
    ...
    for epoch in range(int(args.num_train_epochs)):
                ...
                if (args.save_steps > 0 and global_step % args.save_steps == 0) or \
                    ...
                    if args.evaluate_during_training: 
                        ...
                        with open(evaluate_file, 'r') as f:
                            res = json.load(f)
                        best_score['cider'] = max(best_score['cider'], res['CIDEr'])
                        best_score['r@1'] = max(best_score['r@1'], res['r@1'])
                        res['epoch'] = epoch
                        res['global_step'] = global_step
                        res['best_CIDEr'] = best_score['cider']
                        res['best_R@1'] = best_score['r@1']
                        eval_log.append(res)
                        with open(args.output_dir + '/eval_logs.json', 'w') as f:
                            json.dump(eval_log, f)
        ...
        if  args.save_epochs > 0 and (epoch % args.save_epochs == 0):
            ...
            if args.evaluate_during_training: 
                logger.info("Perform evaluation at step: %d" % (global_step))
                evaluate_file = evaluate(args, val_dataset, model, tokenizer, args.output_dir)
                with open(evaluate_file, 'r') as f:
                    res = json.load(f)
                best_score['cider'] = max(best_score['cider'], res['CIDEr'])
                best_score['r@1'] = max(best_score['r@1'], res['r@1'])
...
def main():
    ...
    parser.add_argument("--caption_file", default='./datasets/{}_prefix_prompts_flt.json', type=str, required=False,
                        help="The caption file for train/val")                  
    ...
    parser.add_argument('--debug', action='store_true',
                        help='fast data loader for debug')
    parser.add_argument('--init_std', type=float, default=0.02,
                        help="random init guassion std")  
    ...
    if args.do_train:
        ...
        logger.info("Load the checkpoint from: %s", args.model_name_or_path)
        ...
        # tokenize ctx vector
        if config.ctx_init: 
            ctx_init_tokens = tokenizer.tokenize(config.ctx_init)
            config.ctx_init_tokens = tokenizer.convert_tokens_to_ids(ctx_init_tokens)
    ...
    # inference and evaluation
        ...
        if not args.do_eval:
            ...
            # predict sentences & calculate coco metric
