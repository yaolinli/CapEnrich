...
import argparse
sys.path += ['metrics/clip_self_retrieve/']
from coco_process_t2i_sim import *

def eval_distinct(gcaps):
    text_dict = {}
    for gcap in gcaps:
        text_dict['COCO_val2014_'+'0'*(12-len(gcap["image_id"]))+gcap["image_id"]] = gcap["caption"]
    ...
    with open('../metrics/clip_self_retrieve/val_total_img_clip_feats_5k.pkl', 'rb') as fin:
...
def evaluate_on_coco_caption(res_file, label_file, outfile=None):
    ...
    # calculate CLIP Retrieve Score
    data = json.load(open(res_file_coco, 'r'))
    r1, r5, r10 = eval_distinct(data)
    result['r@1'], result['r@5'], result['r@10'] = r1, r5, r10
